---
title: "W19 · React 学习清单"
description: "综合练习用状态保存课程列表，用列表渲染生成界面。"
...
> [!abstract] 本课将学到
> 受控状态、数组更新与 key。完成后，你会亲手修改示例并通过自动检查。

## 生活场景切入

综合练习用状态保存课程列表，用列表渲染生成界面。先把它看成解决具体问题的工具，不需要一次记住所有语法。

## 核心概念

**受控状态** 是本课的核心。阅读代码时，先找输入，再看处理规则，最后确认输出。遇到陌生符号时逐行运行，比死记定义更有效。
...
[[programming/web/node-runtime|W20 · Node.js 运行环境]]
