...
- **模型**: Claude Opus 4.8 (1M context)
- **产出**:
  - CET 子板块 C3–C5 三课完成：reading（题型四分类/定位法/同义替换铁律/三陷阱）/ writing（评分四档拆解/句槽弹药库/结构亮点安全排序/五扣分点清单）/ gap（三维差距量化/长难句拆解法/425→500+ 路线）——**CET 子板块全部收官**
  - 共享文件：cet/index.md 五课全部 ✅ + 毕业标准，移除「编写中」提示；english/index.md CET 与 AI 术语状态更新为完成
  - 设计特色：C4 强调句安全性排序接 NLP8 句槽思想；C3 三陷阱与 ML9 指标思维同源；C5 应急裁剪呼应 C1 自测 4——方法论跨课复用显式标注
- **事实核查说明**: 四六级词表规模（4500/6000）、听力语速（140 vs 160-170 wpm）、评分档位描述基于公开的考试大纲与通行备考共识；无具体年份分数数据引用。
- **自检**: 第二十二章清单通过——改动仅限 english 目录六文件；`npx quartz build` 通过（183 文件 294 输出）；三新页 wikilink 全解析、C3/C4「编写中」残留链接已清理并复验；prettier 全绿
- **协作备注**: 基于 main (2d17b82，含 ENG-T-C-001) 切分支 `ai/english/ENG-C3-C5`。至此英语板块全部规划课程竣工（E1-E4 + T1-T4 + C1-C5 + 阅读库）。**全站所有板块课程均已完工。**
- **审查**: 待 REV 审查（建议 REV-ENG-C3-C5）
- **状态**: 完成（待人工审核合并）
...
- **模型**: OpenAI Codex（GPT-5）
- **修正内容**: SQL 与 Web 课程目录的双链竖线被 Markdown 表格误识别为额外列；生成器改用普通站内链接，并新增三列表格结构回归测试
- **自检**: 测试、类型检查、生产构建及浏览器渲染通过
- **审查**: 延续 REV-AIC-PROG-001
- **状态**: 完成（待人工审核合并）

## AIC-PROG-001（Code Lab 路径修正）— 2026-08-25

- **Agent**: AIC Writer（AI 课程 Agent）
- **模型**: OpenAI Codex（GPT-5）
- **修正内容**: Quartz 将 Code Lab 查询参数错误拼接进路径导致课程 iframe 404；全部 44 个实验改用 URL fragment 传递 lesson ID，运行器兼容保留查询参数读取
- **自检**: 77/77 测试、类型检查、生产构建通过；构建产物路径核对正确；Playwright 在 SQL1 课程页确认编辑器正常嵌入且控制台无错误
- **审查**: 延续 REV-AIC-PROG-001
- **状态**: 完成（待人工审核合并）

## QNX-ADULTING-001 — 2026-08-25

- **Agent**: 青年大学习 Writer / Orchestrator
- **模型**: GPT-5 Codex
- **产出**: Adulting 路由与六类信息架构、旧路径 301、统一导航与页脚、集中许可页、内容与构建校验、资产审计、高风险内容整改
- **自检**: 自动测试、类型检查、生产构建与桌面/手机浏览器检查已执行；逐篇原创深度要求未全部通过
- **审查**: REV-QNX-ADULTING-001 — REVISION_REQUIRED（危险内容已关闭，48 篇内容仍过于模板化）
- **状态**: 部分完成；隔离分支不合并、不发布，等待剩余文章逐篇原创重写与再次独立审查
